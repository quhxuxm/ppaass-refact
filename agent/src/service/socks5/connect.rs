...
use common::{
    ...
    PayloadEncryptionType, PayloadType, PrepareMessageFramedResult, PrepareMessageFramedService,
...
pub(crate) struct Socks5ConnectCommandService {
    prepare_message_framed_service: BoxCloneService<TcpStream, PrepareMessageFramedResult, CommonError>,
    ...
    connect_to_proxy_service: BoxCloneService<ConnectToProxyServiceRequest, ConnectToProxyServiceResult, CommonError>,
}
...
impl Socks5ConnectCommandService {
    async fn send_socks5_failure(
        ...
        let connect_result = Socks5ConnectCommandResult::new(Socks5ConnectCommandResultStatus::Failure, None);
    ...
    async fn call_service<T, U>(
        service: BoxCloneService<T, U, CommonError>,
        request: T,
        socks5_client_framed: Framed<&mut TcpStream, Socks5ConnectCodec>,
    ) -> Result<(U, Framed<&mut TcpStream, Socks5ConnectCodec>), CommonError> {
        return match general_call_service(service, request).await {
            Ok(CallServiceResult { result, .. }) => Ok((result, socks5_client_framed)),
            Err(e) => {
...
impl Service<Socks5ConnectCommandServiceRequest> for Socks5ConnectCommandService {
    ...
    fn poll_ready(&mut self, ctx: &mut std::task::Context) -> Poll<Result<(), Self::Error>> {
        ...
        if write_agent_message_service_ready.is_ready() && read_proxy_message_service_ready.is_ready() && connect_to_proxy_service_ready.is_ready() {
    ...
    fn call(&mut self, mut request: Socks5ConnectCommandServiceRequest) -> Self::Future {
        let prepare_message_framed_service = self.prepare_message_framed_service.clone();
        let write_agent_message_service = self.write_agent_message_service.clone();
        let read_proxy_message_service = self.read_proxy_message_service.clone();
        let connect_to_proxy_service = self.connect_to_proxy_service.clone();
        Box::pin(async move {
            ...
            let (connect_to_proxy_service_result, socks5_client_framed) = match connect_command.request_type {
            ...
            let (framed_result, socks5_client_framed) = Self::call_service(
                prepare_message_framed_service,
                connect_to_proxy_service_result.proxy_stream,
                socks5_client_framed,
            ).await?;
            let (write_message_result, socks5_client_framed) = Self::call_service(
            ...
            let (read_proxy_message_result, mut socks5_client_framed) = Self::call_service(
            ...
            let read_proxy_message_result = read_proxy_message_result.ok_or(CommonError::CodecError)?;
