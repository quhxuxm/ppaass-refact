...
impl AgentServer {
    ...
    async fn concrete_run(proxy_addresses: Arc<Vec<SocketAddr>>) -> Result<()> {
        ...
        let agent_rsa_crypto_fetcher = AgentRsaCryptoFetcher::new()?;
    ...
    pub(crate) fn run(&self) -> Result<()> {
        let proxy_addresses_from_config = SERVER_CONFIG.proxy_addresses().as_ref().expect("No proxy addresses configuration item");
        ...
        self.runtime.block_on(Self::concrete_run(proxy_addresses))?;
        Ok(())
    }
}
