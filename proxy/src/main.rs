...
fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt().with_max_level(log_level_filter).with_level(true).with_target(true).with_timer(LogTimer).with_thread_ids(true).with_file(true).with_ansi(false).with_line_number(true).with_writer(non_blocking).init();
